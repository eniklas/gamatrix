...
- An upload only deletes stored titles for platforms it can vouch for: connected in GOG's `PlatformConnections` *and* reporting at least one title (issue #186). Titles from any other platform are carried over untouched, keeping their original `db_updated_at` as the record of when they were last seen. `Repository.delete_user_library_platform()` (surfaced on `/upload`) is the deliberate way to drop them.
